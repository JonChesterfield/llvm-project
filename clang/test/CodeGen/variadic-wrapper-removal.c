...
// RUN: %clang_cc1 -triple i386-unknown-linux-gnu -O1 -emit-llvm -o - %s | opt -S --expand-va-intrinsics | FileCheck %s
// RUN: %clang_cc1 -triple=x86_64-linux-gnu -O1 -emit-llvm -o - %s | opt -S --expand-va-intrinsics | FileCheck %s
...
// RUN: %clang_cc1 -triple=armv6-none--eabi -O1 -emit-llvm -o - %s | opt -S --expand-va-intrinsics | FileCheck %s

// RUN: %clang_cc1 -triple=aarch64-none-linux-gnu -O1 -emit-llvm -o - %s | opt -S --expand-va-intrinsics | FileCheck %s



// expand-va-intrinsics rewrites calls to variadic functions into calls to
