...
template <bool InvertInbox> struct Process {
  ...
  LIBC_INLINE void reset(uint64_t port_count, uint32_t lane_size, void *state) {
    ...
    this->outbox = reinterpret_cast<cpp::Atomic<uint32_t> *>(
        (char *)state + (InvertInbox ? p : s));
    this->packet = reinterpret_cast<Packet *>((char *)state +
  ...
  LIBC_INLINE static constexpr uint64_t
  memory_allocated_mailbox(uint64_t port_count) {
  ...
  LIBC_INLINE static constexpr uint64_t
  memory_offset_buffer(uint64_t port_count) {
