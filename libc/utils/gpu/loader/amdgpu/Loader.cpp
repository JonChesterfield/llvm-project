...
int load(int argc, char **argv, char **envp, void *image, size_t size,
  ...
  void *shared_state;
  if (hsa_status_t err =
          hsa_amd_memory_pool_allocate(finegrained_pool, shared_state_size,
                                       /*flags=*/0, &shared_state))
    handle_error(err);
  hsa_amd_agents_allow_access(1, &dev_agent, nullptr, shared_state);

  // Initialize the RPC server's buffer for host-device communication.
  server.reset(port_size, wavefront_size, shared_state);
  ...
  if (hsa_status_t err = hsa_amd_memory_pool_free(dev_ret))
    handle_error(err);
  if (hsa_status_t err = hsa_amd_memory_pool_free(shared_state))
    handle_error(err);
