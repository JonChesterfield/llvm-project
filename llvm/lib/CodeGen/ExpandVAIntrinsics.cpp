...
public:
  ...
  static void ExpandVAArg(VAArgInst *Inst, const DataLayout &DL) {
    ...
    Value *vaListValue = Builder.CreateLoad(Type::getInt8PtrTy(Ctx), vaListPointer, "arglist_current");


    Align DataAlign = DL.getABITypeAlign(Inst->getType());
    uint64_t DataAlignMinusOne = DataAlign.value() - 1;
  
    Value * Incr = Builder.CreateConstInBoundsGEP1_32(Type::getInt8Ty(Ctx), vaListValue, DataAlignMinusOne);


    Value * Mask = ConstantInt::get(IntPtrTy, ~(DataAlignMinusOne));
    Value *vaListAligned = Builder.CreateIntToPtr(Builder.CreateAnd(Builder.CreatePtrToInt(Incr, IntPtrTy), Mask),
                                                Incr->getType());
  
    auto *Result = Builder.CreateAlignedLoad(Inst->getType(), vaListAligned, DataAlign, "va_arg");
    Result->takeName(Inst);

    Value *Indexes[] = {ConstantInt::get(IntPtrTy, 1)};
    auto *Next = Builder.CreateInBoundsGEP(Inst->getType(), vaListAligned, Indexes, "arglist_next");
 ...
 static  bool isSafeInstruction(Instruction* I)
  {
    if (!I)
      return false;
  ...
  bool canTransformFunctionInIsolation(Function &F) {
    ...
    // Crawl the instructions looking for va_start, looking for functions
    // where the va_list does not escape and thus can be rewritten.
    
    SmallVector<VAStartInst*, 8> StartInstructions;

    // Collect VAStart instructions as the roots of the search
    for (BasicBlock &BB : F) {
      for (Instruction &I : llvm::make_early_inc_range(BB)) {
        if (VAStartInst *II = dyn_cast<VAStartInst>(&I)) {
          StartInstructions.push_back(II);
        }
        if (dyn_cast<VACopyInst>(&I)) {
          // not yet implemented
          return false;
        }
      }
    ...
    SmallVector<AllocaInst*, 8> va_list_allocas;
    for (VAStartInst* I : StartInstructions) {
      Value * arglist = I->getArgList();

      if (auto * II = dyn_cast<AllocaInst>(arglist)) {
        va_list_allocas.push_back(II);
        continue;
      }
      
      if (auto * II = dyn_cast<GetElementPtrInst>(arglist)) {
        Value * op = II->getPointerOperand();
        if (auto * alloca = dyn_cast<AllocaInst>(op)) {
          va_list_allocas.push_back(alloca);
          continue;
        } else {
          return false;
        }
    ...
    for (AllocaInst * I : va_list_allocas)  {
      for (User *U : I->users()) {
        if (isSafeInstruction(dyn_cast<Instruction>(U))) {
          continue;
        }
          
        if (auto * II = dyn_cast<GetElementPtrInst>(U)) {
          for (User* U : II->users()) {
            if (!isSafeInstruction(dyn_cast<Instruction>(U))) {
              return false;
            }
...
static void ExpandCall(Module &M, CallBase* CB, Function *NF) {
    ...
    StructType *VarargsTy = StructType::create(Ctx, LocalVarTypes, (Twine(NF->getName()) + ".vararg").str());
    ...
    auto alloced =
      Builder.Insert(new AllocaInst(VarargsTy, DL.getAllocaAddrSpace(),
                                    nullptr, std::max(MaxFieldAlign, assumedStructAlignment(DL))),
                     "vararg_buffer");
    ...
    for (size_t i = 0; i < Varargs.size(); i++) {
      ...
      Builder.CreateStore(Varargs[i].first, r); // alignment info could be better
    }
