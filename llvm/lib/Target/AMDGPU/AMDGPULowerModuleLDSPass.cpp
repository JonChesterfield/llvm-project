...
public:
  ...
  static Constant *getAddressesOfVariablesInKernel(
    ...
    for (size_t i = 0; i < Variables.size(); i++) {
      ...
      if (LDSVarsToConstantGEP.count(GV) != 0) {
        ...
        // creates the same thing as getPtrToInt to I16
        auto trunc = ConstantExpr::getTrunc(elt, I16);
  ...
  static GlobalVariable *buildLookupTable(
    ...
    size_t NumberVariables = Variables.size();
    size_t NumberKernels = kernels.size();

    ArrayType *KernelOffsetsType =
        ArrayType::get(Type::getInt16Ty(Ctx), NumberVariables);
  ...
  void replaceUsesInInstructionsWithTableLookup(
      Module &M, std::vector<GlobalVariable *> const &ModuleScopeVariables,
    ...
    auto getTableKernelIndex = [&](Function *F) -> Value * {
      if (tableKernelIndexCache.count(F) == 0) {
        LLVMContext &Ctx = M.getContext();
        Type *I32 = Type::getInt32Ty(Ctx);
        FunctionType *FTy = FunctionType::get(I32, {});
        Function *Decl =
    ...
    for (size_t Index = 0; Index < ModuleScopeVariables.size(); Index++) {
      ...
      for (Use &U : make_early_inc_range(GV->uses())) {
        ...
        Value *loaded = Builder.CreateLoad(I16, Address);
        Value *extended = Builder.CreateZExt(loaded, I32);
...
private:
  ...
  void replaceLDSVariablesWithStruct(
      Module &M, DenseSet<GlobalVariable *> const &LDSVarsToTransform,
      LDSVariableReplacement Replacement, PredicateTy Predicate) {
    std::vector<GlobalVariable *> LDSVarsToTransformVec(
        LDSVarsToTransform.begin(), LDSVarsToTransform.end());
    // This indirection makes a mess of the lit tests checking alias information
    // TODO: Put the variables in a constant order before assigning the aliasing
    // info
    replaceLDSVariablesWithStructVec(M, LDSVarsToTransformVec, Replacement,
                                     Predicate);
  }
  ...
  void replaceLDSVariablesWithStructVec(
    ...
    // A hack... need to insert the aliasing info in a predictable order for
    // lit tests. TODO: Have them in a reasonable order already, ideally the
    // same order they get allocated
