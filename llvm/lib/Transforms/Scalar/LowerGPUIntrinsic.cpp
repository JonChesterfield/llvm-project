...
namespace range {
// Range annotation is a work in progress
typedef void (*rangeMapMod)(int &lower, int &upper);
struct RangeMap {
  ...
  operator bool() const { return Intrinsic != Intrinsic::not_intrinsic; }
};

// Lots of the nvptx ones have range metadata on them, some need runtime calls
static const constexpr RangeMap lsr[] = {
    {Intrinsic::amdgcn_workitem_id_x, 0, 1024, nullptr},
    {Intrinsic::amdgcn_workitem_id_y, 0, 1024, nullptr},
    {Intrinsic::amdgcn_workitem_id_z, 0, 1024, nullptr},

    {Intrinsic::nvvm_read_ptx_sreg_laneid, 0, 32, nullptr},

    {Intrinsic::nvvm_read_ptx_sreg_warpsize, 32, 32,
     nullptr}, // reasonable? doesn't seem to be folding
    {Intrinsic::nvvm_read_ptx_sreg_nctaid_y, 1, 65536, nullptr},
    //{Intrinsic::nvvm_read_ptx_sreg_nctaid_z, 1, 65536, nullptr},
};

template <Intrinsic::ID ID> constexpr RangeMap getRangeRet() {
  enum { lsr_size = sizeof(lsr) / sizeof(lsr[0]) };
  for (size_t i = 0; i < lsr_size; i++) {
    if (lsr[i].Intrinsic == ID)
      return lsr[i];
  }
  return {Intrinsic::not_intrinsic, 0, 0, nullptr};
}
...
bool S(Module &M, IRBuilder<> &, Intrinsic::ID from, CallBase *CI) {
  ...
  constexpr range::RangeMap RM = range::getRangeRet<To>();
  if (RM) {
    int Lower = RM.lower;
    int Upper = RM.upper;
    ...
    ConstantRange OldRange =
        CI->getRange().value_or(ConstantRange::getFull(BitWidth));
    ;
    ConstantRange Range =
        (Lower == Upper) // Otherwise it asserts
            ? ConstantRange(APInt(BitWidth, Lower))
            : ConstantRange(APInt(BitWidth, Lower), APInt(BitWidth, Upper));
    ...
    CI->addRetAttr(llvm::Attribute::AttrKind::NoUndef);
  }
...
// num_blocks on amdgpu is amdgcn_grid_size / amdgcn_workgroup_size
// num_blocks on amdgpu is amdgcn_grid_size / builtin_gpu_num_threads
// where grid size is it's own thing with lowering in CGBuiltin at present
// moving it here allows writing num_blocks in terms of grid size but
// making grid_size an intrinsic in this patch might be contentious
...
#if 0
  // MaxOpenCLWorkGroupSize is 1024 but hidden away in clang's targetinfo,
  // ignore it for now. Plus why is it "opencl" in particular?
...
static const IntrinsicMap ls[] = {
    ...
    // Pick up the range annotation. Ugly, maybe worse than CGBuiltin?
    // {amdgcn_workitem_id_x, S<amdgcn_workitem_id_x>, nullptr},
    // {amdgcn_workitem_id_y, S<amdgcn_workitem_id_y>, nullptr},
    // {amdgcn_workitem_id_z, S<amdgcn_workitem_id_z>, nullptr},
    ...
    // CGBuiltin allows the above to be implemented partly in terms of
    // amdgcn_grid_size.
